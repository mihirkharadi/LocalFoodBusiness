...
useEffect(()=>
{
  ...
  const q = query(collection(db, "chats", chatId, "messages"), orderBy("timeStamp","desc"),limit(7));
    ...
    <div className="flex-1 max-h-[66vh] overflow-y-auto no-scrollbar p-4 space-y-3">
      {messages.map((msg) => (
        <div key={msg.id} className={`flex ${msg.senderId === kitchenId ? "justify-end" : "justify-start"}`}>
          <div className={`p-3 rounded-lg max-w-xs ${msg.senderId === kitchenId ? "bg-green-500 text-white" : "bg-gray-200 text-black"}`}>
            <h1 className="text-sm text-black text-left">{kitchenName} (Vendor)</h1>
            <p>{msg.text}</p>
          </div>
        </div>
      ))}
